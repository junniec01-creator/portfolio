...
`data/awards.ts`의 `description`에서 줄바꿈은 `\n`, 강조는 `**텍스트**`로 표기하면 화면에 반영됩니다. `rank` 값(최우수 / 우수 / 장려)은 카드에 배지로 표시됩니다.

### 링크 추가

`components/sections/Contact.tsx`의 `LINKS` 배열에서 GitHub · Instagram 의 `href`를 채우면 해당 링크가 화면에 나타납니다. 비어 있는 동안에는 렌더링되지 않습니다.
